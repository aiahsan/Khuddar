...
import {
    ScrollView, View, TouchableOpacity, StyleSheet, Text, RefreshControl,
} from 'react-native'
...
const MyComponent = () => {
    ...
    const fetchFamily = async function () {
        try {
            ...
            const { data, status } = await repository.my_tests({ headers: { 'secret': userr != null && userr.token && userr.token.token ? userr.token.token : "", 'user_id': userr != null && userr.id ? userr.id : '' } }).then(x => x).then(x => x)
            console.log(data)
            if (status == 200 && data && data.success == true) {


                if (data.response && data.response.my_tests) {

                    setmembers(data.response.my_tests)
                    setsearchmembers(data.response.my_tests)
                }
        ...
        return <View style={[styles.felxRow, { justifyContent: 'space-between', borderBottomWidth: 1, borderColor: '#ededed', padding: 10 }]}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                ...
                <View>
                    ...
                    {/* <Heading head={name} style={{ color: '#717171', fontSize: 14, marginLeft: 12 ,marginTop:RFValue(-20)}} /> */}
                    <View style={{ marginLeft: 12,marginTop:RFValue(-20) }}>
                        <Heading head="Test Description :" style={{ color: '#717171', fontSize: 14 }} type={true} />
                        <Heading head={type} style={{ color: '#717171', fontSize: 14, marginTop: RFValue(-20) }} />
                        <Heading head="Assign To :"style={{ color: '#717171', fontSize: 14, marginTop: RFValue(-20) }} />
    ...
    return (<View style={[{ backgroundColor: 'white', flex: 1 }]}>
        ...
        <View style={[styles.ctr, styles.mt5]}>
            <ScrollView refreshControl={
                ...
                <Heading head="Khuddar Lab Tests" label="Description...." />
                ...
                </View>
                {
                    ...
                    /> : searchmembers.map((x, i) => <ResultView key={i} image={x.doctor && x.doctor.image ? x.doctor.image : ""} name={x.doctor && x.doctor.name ? x.doctor.name : ""} type={x.test_description ? x.test_description : ""} />)
                }
            </ScrollView>
