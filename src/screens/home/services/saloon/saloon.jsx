...
const DisplayingErrorMessagesSchema = Yup.object().shape({
  ...
  saloon_name: Yup.string().required('Required'),
  saloon_location: Yup.string().required('Required'),
...
const MyComponent = () => {
    ...
    const postData = async (datapost) => {
        ...
        try {
            const { data, status } = await repository.saloon_add(datapost,{ headers: { 'secret': userr!=null&&userr.token&&userr.token.token?userr.token.token:"", 'user_id': userr!=null&&userr.id?userr.id:'' } }).then(x => x).then(x => x)
        ...
        {({ errors, touched, handleChange, handleBlur, values, handleSubmit, setFieldValue }) => {
            ...
            return <View style={[{ backgroundColor: 'white', flex: 1 }]}>
                ...
                <ScrollView>
                    ...
                    <Heading head="Saloon Details" label="Upload Details about Saloon" />
                    ...
                    {/* <TextInput
                                label="Saloon Name"
                                mode='outlined'
                                onChangeText={handleChange('saloon_name')}
                                onBlur={handleBlur('saloon_name')}
                        ...
                        {touched.saloon_name && errors.saloon_name && <Text style={{ color: 'red', marginTop: 10 }}>{errors.saloon_name}</Text>}
                    <TextInput
                                label="Saloon Location"
                                mode='outlined'
                                onChangeText={handleChange('saloon_location')}
                                onBlur={handleBlur('saloon_location')}
                                value={values.saloon_location}
                                style={[styles.textInput, { width: '100%',marginTop:15 }]}
                                left={
                                    <TextInput.Icon
                                        name={() => <AntDesign name="user" size={16} style={[styles.themeColor, { paddingTop: '30%' }]} />}
                        ...
                        {touched.saloon_location && errors.saloon_location && <Text style={{ color: 'red', marginTop: 10 }}>{errors.saloon_location}</Text>} */}
                        ...
                        <TouchableOpacity disabled={showAnimation} mode="contained" style={[styles.buttonMain, styles.mt5, { marginBottom: 20 }, styles.themeBackColor, styles.border1]} onPress={handleSubmit}>
                            ...
                            /> : <Text style={styles.textTr, styles.themeColorwhite, { height: 30, textAlign: 'center', marginTop: 5, marginBottom: 5, color: 'white', fontSize: 18, fontFamily: 'PoppinsMedium', color: 'white' }}>Register Saloon</Text>
                            }
                        </TouchableOpacity>
                        {show && (
                            <DateTimePicker
