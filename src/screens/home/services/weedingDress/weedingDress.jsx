...
const DisplayingErrorMessagesSchema = Yup.object().shape({
  ...
  banquet_name: Yup.string().required('Required'),
  banquet_location: Yup.string().required('Required'),
...
const MyComponent = () => {
    ...
    const postData = async (datapost) => {
        ...
        try {
            const { data, status } = await repository.wedding_add(datapost,{ headers: { 'secret': userr!=null&&userr.token&&userr.token.token?userr.token.token:"", 'user_id': userr!=null&&userr.id?userr.id:'' } }).then(x => x).then(x => x)
            ...
            return <View style={[{ backgroundColor: 'white', flex: 1 }]}>
                ...
                <ScrollView>
                    ...
                    {/* <TextInput
                                label="Banquet Name"
                                mode='outlined'
                                onChangeText={handleChange('banquet_name')}
                                onBlur={handleBlur('banquet_name')}
                        ...
                        {touched.banquet_name && errors.banquet_name && <Text style={{ color: 'red', marginTop: 10 }}>{errors.banquet_name}</Text>}
                    <TextInput
                                label="Banquet Location"
                                mode='outlined'
                                onChangeText={handleChange('banquet_location')}
                                onBlur={handleBlur('banquet_location')}
                                value={values.banquet_location}
                                style={[styles.textInput, { width: '100%',marginTop:15 }]}
                                left={
                                    <TextInput.Icon
                                        name={() => <AntDesign name="user" size={16} style={[styles.themeColor, { paddingTop: '30%' }]} />}
                        ...
                        {touched.banquet_location && errors.banquet_location && <Text style={{ color: 'red', marginTop: 10 }}>{errors.banquet_location}</Text>} */}
                        ...
                        <TouchableOpacity disabled={showAnimation} mode="contained" style={[styles.buttonMain, styles.mt5, { marginBottom: 20 }, styles.themeBackColor, styles.border1]} onPress={handleSubmit}>
                            ...
                            /> : <Text style={styles.textTr, styles.themeColorwhite, { height: 30, textAlign: 'center', marginTop: 5, marginBottom: 5, color: 'white', fontSize: 18, fontFamily: 'PoppinsMedium', color: 'white' }}>Register Wedding</Text>
                            }
                        </TouchableOpacity>
                        {show && (
                            <DateTimePicker
