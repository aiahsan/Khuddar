...
const MyComponent = () => {
   ...
   const fetchFamily = async function () {
      try {
         ...
         const { data, status } = await repository.getservices("rent_a_car", { headers: { 'secret': userr != null && userr.token && userr.token.token ? userr.token.token : "", 'user_id': userr != null && userr.id ? userr.id : '' } }).then(x => x).then(x => x)
         console.log(data)
   ...
   return (<View style={[{ backgroundColor: 'white', flex: 1 }, styles.ctr]}>
      ...
      <ScrollView refreshControl={
         ...
         <Heading head="Avaiable Rent A Car" label="Select Rent A Car" type={true} />
         {
            ...
            /> : newsList.map(x => <NewsCard handlePress={() => handleSelectService(x.id)} date_created={x.date_created ? x.date_created : ""} short_content={x.description ? x.description : ""} title={x.title ? x.title : ""} hasButton={true} src={x.image ? x.image : ""} />
            )
         }
