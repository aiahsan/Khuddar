...
const DisplayingErrorMessagesSchema = Yup.object().shape({
    ...
    rentacar_name: Yup.string().required('Required'),
    rentacar_location: Yup.string().required('Required'),
...
const MyComponent = () => {
    ...
    const postData = async (datapost) => {
        ...
        try {
            const { data, status } = await repository.rentacar_add(datapost, { headers: { 'secret': userr != null && userr.token && userr.token.token ? userr.token.token : "", 'user_id': userr != null && userr.id ? userr.id : '' } }).then(x => x).then(x => x)
            ...
            return <View style={[{ backgroundColor: 'white', flex: 1 }]}>
                ...
                <ScrollView>
                    ...
                    <View style={[styles.ctr, styles.mt5]}>
                        <Heading head="Rent A Car Details" label="Upload Details about Rent A Car" />
                        ...
                        {/* <TextInput
                                ...
                                mode='outlined'
                                onChangeText={handleChange('rentacar_name')}
                                onBlur={handleBlur('rentacar_name')}
                                value={values.rentacar_name}
                                style={[styles.textInput, { width: '100%',marginTop:15 }]}
                                left={
                                    <TextInput.Icon
                                        name={() => <AntDesign name="user" size={16} style={[styles.themeColor, { paddingTop: '30%' }]} />}
                        ...
                        {touched.rentacar_name && errors.rentacar_name && <Text style={{ color: 'red', marginTop: 10 }}>{errors.rentacar_name}</Text>} */}
                        {/* <TextInput
                                label="Rent A car Location"
                                mode='outlined'
                                onChangeText={handleChange('rentacar_location')}
                                onBlur={handleBlur('rentacar_location')}
                                value={values.rentacar_location}
                                style={[styles.textInput, { width: '100%',marginTop:15 }]}
                                left={
                                    <TextInput.Icon
                                        name={() => <AntDesign name="user" size={16} style={[styles.themeColor, { paddingTop: '30%' }]} />}
                        ...
                        {touched.rentacar_location && errors.rentacar_location && <Text style={{ color: 'red', marginTop: 10 }}>{errors.rentacar_location}</Text>} */}
                        ...
                        <TouchableOpacity disabled={showAnimation} mode="contained" style={[styles.buttonMain, styles.mt5, { marginBottom: 20 }, styles.themeBackColor, styles.border1]} onPress={handleSubmit}>
                            ...
                            /> : <Text style={styles.textTr, styles.themeColorwhite, { height: 30, textAlign: 'center', marginTop: 5, marginBottom: 5, color: 'white', fontSize: 18, fontFamily: 'PoppinsMedium', color: 'white' }}>Register Rent A car</Text>
                            }
                        </TouchableOpacity>
                        {show && (
                            <DateTimePicker
