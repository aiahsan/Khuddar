...
const MyComponent = () => {
   ...
   const fetchFamily = async function () {
      try {
         ...
         const { data, status } = await repository.getservices("baraat", { headers: { 'secret': userr != null && userr.token && userr.token.token ? userr.token.token : "", 'user_id': userr != null && userr.id ? userr.id : '' } }).then(x => x).then(x => x)
         console.log(data)
   ...
   return (<View style={[{ backgroundColor: 'white', flex: 1 }, styles.ctr]}>
      ...
      <ScrollView refreshControl={
         ...
         <Heading head="Avaiable Barat" label="Select Barat" type={true} />
         {
