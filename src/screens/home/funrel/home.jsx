...
import { ScrollView, View, TouchableOpacity, Text, Image,Linking ,Platform} from 'react-native'
...
const MyComponent = () => {
    ...
    let [toolFree,settoolFree]=React.useState("");
    ...
    const postData = async (datapost) => {
        ...
        try {
            const { data, status } = await repository.funeral(datapost, { headers: { 'secret': userr != null && userr.token && userr.token.token ? userr.token.token : "", 'user_id': userr != null && userr.id ? userr.id : '' } }).then(x => x).then(x => x)
    ...
    const fetchFamily = async function () {
        try {
            ...
            const { data, status } = await repository.list_family_member().then(x => x).then(x => x)
            console.log(data)
    ...
    const fetchToolFree = async function () {
        try {

            const { data, status } = await repository.toll_free_number().then(x => x).then(x => x)
            console.log(data)
            if (status == 200 && data && data.success == true) {


                if (data.response && data.response.number) {

                   settoolFree(data.response.number);
                }
                // dispatch(Add_All_members(data.response.user.members))
                setshowAnimation(false);
            }
            ...
            return <View style={[{ backgroundColor: 'white', flex: 1 }]}>
                ...
                <ScrollView>
                    ...
                    <View style={[styles.ctr, styles.mt5]}>
                        <Heading head="Funeral Registration Details" label="Upload Details about funeral" />
                        {/* <Image source={require('../../../images/funrel/funrel.png')} style={{width:'100%',resizeMode:'contain',height:70}}/> */}
                        ...
                        <Text style={{ textAlign: 'center', fontSize: 16,fontFamily:'PoppinsMedium' }}>Family code</Text>
                      
                        <TouchableOpacity onPress={()=>openDialScreen()} style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop: 10, marginBottom: 10 }}>
                            <Image style={{ width: 50, height: 50 }} source={{ uri: 'https://image.flaticon.com/icons/png/512/724/724664.png' }} />


                        </TouchableOpacity>
                        <Text style={{ textAlign: 'center', fontSize: 16,fontFamily:'PoppinsMedium' }}>Tool Free :{toolFree}</Text>
