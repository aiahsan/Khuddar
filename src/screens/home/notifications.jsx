...
const MyComponent = () => {
  ...
  const fetchFamily= async function () {
   try {
       ...
       const { data, status } = await repository.notifications_list({ headers: { 'secret': userr!=null&&userr.token&&userr.token.token?userr.token.token:"", 'user_id': userr!=null&&userr.id?userr.id:'' } }).then(x => x).then(x => x)
           ...
           if (data.response.notifications)
               {
                   ...
                   setnewsList(data.response.notifications)
 ...
 <ScrollView  refreshControl={
   ...
   <Heading head="Notifications" label="Recent Notifications" type={true} />
               ...
               />:newsList.map((x,i)=> <NotificationCard   userName={x.title} datetime={moment(x.date_created).format("HH:mm")}  notification={x.description} key={i}/>
               )
 }
 </ScrollView>
    </View>);
