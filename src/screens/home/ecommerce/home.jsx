...
import {eAll_Mart_Items} from '../../../redux/actions/emartActionMethodes.';
...
const MyComponent = () => {
    ...
    const familyMembers=useSelector(x=>x.emartItems);
  ...
  const onRefresh = React.useCallback(() => {
   ...
   const fetchFamily= async function () {
        try {
            ...
            const { data, status } = await repository.emart_products({ headers: { 'secret': userr!=null&&userr.token&&userr.token.token?userr.token.token:"", 'user-id': userr!=null&&userr.id?userr.id:'' } }).then(x => x).then(x => x)
            console.log(data)
                ...
                if (data.response.products)
                    {
                        ...
                        setmembers(data.response.products)
                        dispatch(eAll_Mart_Items(data.response.products))
                    }
    ...
    const ResultView = ({ image, name, type,x }) => {
      ...
      <Paragraph numberOfLines={1} style={[styles.mainTitleHead1,{fontSize:12,marginTop:RFValue(-6)}]}>Price: RS-<Text style={x.discounted_price?{textDecorationLine: 'line-through', textDecorationStyle: 'solid'}:{}}>{x.price?x.price:""}</Text></Paragraph>
      <Paragraph numberOfLines={1} style={[styles.mainTitleHead1,{fontSize:12,marginTop:RFValue(-6)}]}>RS-{x.discounted_price?x.discounted_price:""}</Paragraph>
    </Card.Content>
      </Card>
        </View>
    }
    return (<View style={[{ backgroundColor: 'white', flex: 1 }]}>
       ...
       alwaysBounceVertical showsVerticalScrollIndicator={false} style={{ marginBottom: RFValue(138) }}>
                <Heading head="E-Commerce" label=" Buy best products" />
...
{
                         members.map((x, i) => <TouchableOpacity onPress={()=>navigation.navigate(Nav.EProduct,{x:x})} style={[Cardstyles.box]} key={i}><ResultView i={i}  x={x} image={x.images ? x.images : []} name={x.title ? x.title : ""} type={x.type ? x.type : ""} /></TouchableOpacity>)
}
                 </View>
                }
            </ScrollView>
