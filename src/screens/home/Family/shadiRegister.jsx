...
const MyComponent = () => {
    ...
    const postData = async (datapost) => {
        ...
        try {
            ...
            const { data, status } = await repository.nikkah_add_main(datapost,{ headers: { 'secret': userr!=null&&userr.token&&userr.token.token?userr.token.token:"", 'user_id': userr!=null&&userr.id?userr.id:'' } }).then(x => x).then(x => x)
            ...
            return <View style={[{ backgroundColor: 'white', flex: 1 }]}>
                ...
                <ScrollView>
                    ...
                    <View style={[styles.ctr, styles.mt5]}>
                        <Heading head="Shadi Registration Details" label="Upload Details about your shadi" />
                        ...
                        {touched.date && errors.date && <Text style={{ color: 'red', marginTop: 10 }}>{errors.date}</Text>}
                        ...
                        <TouchableOpacity disabled={showAnimation} mode="contained" style={[styles.buttonMain, styles.mt5, { marginBottom: 20 }, styles.themeBackColor, styles.border1]} onPress={handleSubmit}>
                            ...
                            /> : <Text style={styles.textTr, styles.themeColorwhite, { height: 30, textAlign: 'center', marginTop: 5, marginBottom: 5, color: 'white', fontSize: 18, fontFamily: 'PoppinsMedium', color: 'white' }}>Register Shadi</Text>
                            }
                        </TouchableOpacity>
                        {show && (
                            <DateTimePicker
