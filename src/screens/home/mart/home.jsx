...
import {All_Mart_Items} from '../../../redux/actions/martActionMethodes';
...
const MyComponent = () => {
    ...
    const familyMembers=useSelector(x=>x.martItems);
  ...
  const onRefresh = React.useCallback(() => {
   ...
   const fetchFamily= async function () {
        try {
            ...
            const { data, status } = await repository.mart_products({ headers: { 'secret': userr!=null&&userr.token&&userr.token.token?userr.token.token:"", 'user_id': userr!=null&&userr.id?userr.id:'' } }).then(x => x).then(x => x)
            console.log(data)
    ...
    return (<View style={[{ backgroundColor: 'white', flex: 1 }]}>
       ...
       alwaysBounceVertical showsVerticalScrollIndicator={false} style={{ marginBottom: RFValue(138) }}>
                <Heading head="Mart" label=" Buy best products" />
...
{
                         members.map((x, i) => <TouchableOpacity onPress={()=>navigation.navigate(Nav.Product,{x:x})} style={[Cardstyles.box]} key={i}><ResultView i={i}  x={x} image={x.images ? x.images : []} name={x.title ? x.title : ""} type={x.type ? x.type : ""} /></TouchableOpacity>)
}
                 </View>
                }
            </ScrollView>
