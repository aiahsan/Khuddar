...
export const cart = (state =mainCart, action) => {
    
    if (action.type == types.addCartItem) {
    ...
    else if (action.type==types.removeCartItem)
    {
        let cartItem=_.find(state.items,x=>x.id==action.payload.id);
        let item=action.payload;
    ...
    else if (action.type==types.removeSingleCartItem)
    {
        let item=action.payload;
        state.items= state.items.filter(x=>x.id!=item.id)
                state.totalItems=state.totalItems-1;
    ...
    else if(action.type==types.emptyCart)
    {
